...
import { createTransfer, updateTransfer } from "../../services"
...
// Edit mode is entered from the transaction list (a transfer leg). The list
// passes the transfer's normalized shape via navigation state so we can prefill
...
const AccountTransfer = () => {
  ...
  const editState = (location.state ?? {}) as TransferEditState
  const { theme } = useTheme()
  const styles = useThemedStyles(makeStyles)
  const insets = useSafeAreaInsets()
  const { accounts } = useAccounts()

  const [fromId, setFromId] = useState(editState.fromId ?? "")
  const [toId, setToId] = useState(editState.toId ?? "")
  const [amountText, setAmountText] = useState(
    editState.value != null ? String(editState.value) : "",
  )
  const [description, setDescription] = useState(editState.description ?? "")
  const [picker, setPicker] = useState<PickerField>(null)
  const [submitting, setSubmitting] = useState(false)

  // Preserve the original date on edit; new transfers are stamped now.
  const transferDate = useMemo(
    () => (editState.date ? new Date(editState.date) : new Date()),
    [editState.date],
  )
    ...
    try {
      ...
      if (isEditing) {
        ...
        notify.success(
          ...
          `Ambos movimientos quedaron en $${amount.toLocaleString("es-CO")}.`,
        )
        navigate(-1)
      } else {
      ...
      const message = isApiError(error)
        ? error.message
        : isEditing
        ? "No pudimos actualizar la transferencia. Intentá de nuevo."
    ...
    <View style={[styles.screen, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>
          {isEditing ? "Editar transferencia" : "Transferir"}
        </Text>
      ...
      >
        ...
        >
          <Text style={styles.caption}>
            {isEditing
              ? "Se actualizan los dos movimientos de la transferencia a la vez para que nunca queden desfasados."
