...
export const getBudget = async (year: number, month: number): Promise<BudgetStatus> => {
...
export const saveBudget = async (dto: BudgetUpsertDTO): Promise<Budget> => {
  const { data } = await client.put<{ data: Budget }>("/budget", dto as unknown as Record<string, any>)
  return data
}
