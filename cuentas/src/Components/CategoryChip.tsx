...
const sizeConfig: Record<Size, { height: number; radius: number; iconSize: number; fontSize: number }> = {
...
export default function CategoryChip({ categoryId, name, icon, size = 'md' }: Props) {
  const tone = getTone(categoryId);
  const cfg = sizeConfig[size];

  return (
    <View style={[styles.chip, { backgroundColor: tone.bg, borderRadius: cfg.radius, height: cfg.height, paddingHorizontal: cfg.height * 0.35 }]}>
      {icon ? (
        <Ionicons name={icon as any} size={cfg.iconSize} color={tone.fg} />
      ) : (
        <Text style={[styles.initial, { color: tone.fg, fontSize: cfg.iconSize }]}>
