...
describe("TransactionAggregateService", () => {
	test("Aggregates transactions correctly", () => {
		...
		expect(result.length).toBe(expectedTransactionAggregates.length);
		expect(result).toEqual(expect.arrayContaining(expectedTransactionAggregates));
	});
});
